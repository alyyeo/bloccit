...
RSpec.describe VotesController, type: :controller do
    ...
    context "signed in user" do
        ...
        describe "POST up_vote" do
            it "the user's first vote increases the number of post votes by 1" do
            ...
            it ":back redirects to the post show page" do
                # request.env["HTTP_REFERER"] = topic_post_path(my_topic, user_post)
                post :up_vote, format: :js, post_id: user_post.id
        ...
        describe "POST down_vote" do
            ...
            it ":back redirects to the topic show page" do
                # request.env["HTTP_REFERER"] = topic_path(my_topic)
                post :down_vote, format: :js, post_id: user_post
                # expect(response).to redirect_to(my_topic)
                expect(response).to have_http_status(:success)
            end
        end
